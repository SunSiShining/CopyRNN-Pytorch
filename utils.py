...
def init_results(topk_range=[5,10], score_names=['precision', 'recall', 'f_score'], keys=['exact', 'soft'], return_num = False):
    ...
    if return_num:
        ...
        for topk in topk_range:
            for name in score_names:
                for key in keys:
                        result['%s@%d_%s' %(name, topk, key)] = 0
    else:
        for topk in topk_range:
            for name in score_names:
                for key in keys:
                        result['%s@%d_%s' %(name, topk, key)] = []
